#!/usr/bin/env python3
"""
Cloud Run Job — Mentores GCS→GCS (prepare | worker) entre proyectos.
...
Orquestación: Cloud Workflows (mentores_gcs_copy/workflows/daily_pipeline.yaml)
"""
...
def run_worker(config: dict[str, Any]) -> int:
    ...
    manifest_prefix = job_cfg.get("manifest_prefix", "state/manifests/mentores")
...
def main() -> int:
    ...
    print(f"[mentores_gcs_copy] role={role}")
    if role == "prepare":
        run_prepare(config)
        return 0
