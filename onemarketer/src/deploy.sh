...
PAYLOAD='{"current_date": "YESTERDAY", "days_back": 0, "force_reprocess": false}'  # force_reprocess=true solo reproceso manual
...
if [[ -n "${GCP_GCS_PATH:-}" ]]; then
    GCP_ENV_VARS="${GCP_ENV_VARS},GCP_GCS_PATH=${GCP_GCS_PATH}"
    print_info "GCP_GCS_PATH=${GCP_GCS_PATH}"
fi
