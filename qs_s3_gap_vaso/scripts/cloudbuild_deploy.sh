#!/usr/bin/env bash
# Deploy Cloud Run Job qs_s3_gap_vaso (legado-miss: .audio ∪ prefijo≠6).
set -euo pipefail
...
SOURCE_DIR="${_SOURCE_DIR:-qs_s3_gap_vaso/src}"
...
echo "=== Deploy OK ==="
echo "Job vaso (no toca qs_s3_to_gcs ni catalog prod)."
echo "1) gap_prepare:"
echo "  gcloud run jobs execute ${_JOB_NAME} --region=${LOCATION} --project=${_PROJECT_ID} \\"
echo "    --tasks=1 --update-env-vars=QS_JOB_ROLE=gap_prepare,GAP_TARGET_DATE=YYYY-MM-DD"
echo "2) worker (N = count del manifiesto):"
echo "  gcloud run jobs execute ${_JOB_NAME} --region=${LOCATION} --project=${_PROJECT_ID} \\"
